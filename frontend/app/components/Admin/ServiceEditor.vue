      ...
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-[var(--slate-500)]">
            Maintain service details, scopes, and content.
          </p>
        </div>
      ...
      >
        ...
        <el-form-item label="Content (SFDT)">
          <ClientOnly>
            ...
            <template #fallback>
              <div
                class="rounded-2xl border border-[var(--slate-100)] bg-white p-6 text-sm text-[var(--slate-500)]"
              >
                Loading editor...
              </div>
            </template>
...
const title = computed(() =>
  isEditing.value ? "Edit Service" : "Add Service"
);
  ...
  await formEl.validate(async (valid) => {
    if (valid) {
      if (form.thumbnail && !resolveAttachmentId(form.thumbnail)) {
        ElMessage.warning(
          "Upload thumbnail belum terhubung, simpan tanpa thumbnail."
        );
      }
      ...
      try {
        ...
        if (isEditing.value && props.serviceId) {
          const updated = await apiFetch<ServiceApi>(
          ...
          const refreshed = await apiFetch<ServiceApi>(
            `/services/${updated.id}`
          );
