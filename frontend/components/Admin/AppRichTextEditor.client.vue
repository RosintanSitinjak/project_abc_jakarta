    ...
    if (response && response.url) {
      if (args.operation === 'upload') {
        ...
        setTimeout(() => {
          ...
          let changed = false;
          const imgElements = rteObj.value.$el.querySelectorAll('img.e-rte-image');
          imgElements.forEach((img: HTMLImageElement) => {
            if (img.src.includes('blob:') || img.getAttribute('alt') === fileUploadName || img.src.includes(fileUploadName)) {
